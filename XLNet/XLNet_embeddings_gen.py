...
mpstDF_processsed["processed_synopsis_t1"] = mpstDF_processsed["plot_synopsis"].apply(
    lambda x: decontracted(" ".join(preprocess_string(x, [lambda x: x.lower(), strip_tags]))))
# Type 2 Decontracted Text Stop Words Removed
mpstDF_processsed["processed_synopsis_t2"] = mpstDF_processsed["plot_synopsis"].apply(
    lambda x: decontracted(" ".join(preprocess_string(x, [lambda x: x.lower(), strip_tags, remove_stopwords]))))
...
# %%
mpstDF_processsed["processed_synopsis_t1_short"] = mpstDF_processsed["processed_synopsis_t1"].apply(
    lambda x: truncateText(x, 250))
# Type 2 Decontracted Text Stop Words Removed
mpstDF_processsed["processed_synopsis_t2_short"] = mpstDF_processsed["processed_synopsis_t2"].apply(
    lambda x: truncateText(x, 300))
...
def get_Encodings(text, tokenizer=tokenizer, model=model, verbose=False):
    ...
    with torch.no_grad():
        outputs = model(text_encoding_tensor,
...
def getXLNetEmbeddings(df, column, verbose=False):
    embeddings = [np.array(get_Encodings(x, verbose=verbose))
                  for x in df[column]]
...
# %%
sampleDF = mpstDF_processsed[[
    "processed_synopsis_t1", "processed_synopsis_t2"]].head(3)
sampleDF["processed_synopsis_t1"] = sampleDF["processed_synopsis_t1"].apply(
    lambda x: x[:1000])
sampleDF["processed_synopsis_t2"] = sampleDF["processed_synopsis_t2"].apply(
    lambda x: x[:1000])
# display(sampleDF)
...
# %%
# ,"When questioned by three tough-talking INS officials, they notice a tattoo on Tony's left arm of a black heart with a pitchfork through it, which identifies him as a hitman, and detain him in a camp called 'Freedomtown' with other Cubans"])
tokenizer.encode("In May 1980, a Cuban man named Tony Montana (Al Pacino) claims asylum, in Florida, USA, and is in search of the \"American Dream\" after departing Cuba in the Mariel boatlift of 1980.")
...
xlnet_embeddings_t1 = getXLNetEmbeddings(
    mpstDF_processsed, "processed_synopsis_t1_short")
...
xlnet_embeddings_t2 = getXLNetEmbeddings(
    mpstDF_processsed, "processed_synopsis_t2_short")
